...
class Cityscapes15KWeightedDataset(Pix2pixDataset):
    ...
    def get_paths(self, opt):
        ...
        with open('%s/cityscapes_15k_weighted_lbl_info_%s.txt'%(root, phase),'r') as f:
            label_paths_all = f.readlines()

        label_paths = [p.strip() for p in label_paths_all]

        with open('%s/cityscapes_15k_weighted_im_info_%s.txt'%(root, phase),'r') as f:
